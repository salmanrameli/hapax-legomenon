                ...
                <Col sm={12}>
                    <div className="d-flex bg-white gap-2 flex-wrap justify-content-center p-3 border-hapax-secondary hapax-box-shadow rounded-4">
                        <div className="d-flex" style={{height: "200px", width:"100%", overflowY:"scroll"}}>
                            <textarea disabled={mode == GenerateMode.MODE_GENERATING_IMAGE} placeholder="Type your prompt here or create one using the button below" style={{width:"100%", height:"100%", resize:"none", boxSizing:"border-box", border:"none", outline:"none"}} value={prompt} onChange={(e) => mode !== GenerateMode.MODE_GENERATING_IMAGE && setPrompt(e.target.value)} />
                        </div>
                    </div>
                </Col>
                <Col sm={12} md={4}>
                    ...
                    <br /><br />
                    <Button size="lg" className="w-100 btn-hapax-danger rounded-4" disabled={mode == GenerateMode.MODE_GENERATING_PROMPT || mode == GenerateMode.MODE_GENERATING_IMAGE || prompt == "" || prompt == undefined} onClick={generateImage}>{mode == GenerateMode.MODE_GENERATING_IMAGE ? BTN_IMAGE_GENERATING_TEXT : BTN_IMAGE_DEFAULT_TEXT} {mode == GenerateMode.MODE_GENERATING_IMAGE ? <Spinner animation="border" variant="danger" style={{ width: '1.5rem', height: '1.5rem', borderWidth: '0.3em' }} /> : <PlayFill />}</Button>
                    <br /><br />
